defmodule Gotham.WorkingTimeControllerTest do
  ...
  describe "workingtimes" do
    ...
    test "create_working_time/1 with valid data creates a working_time" do
      valid_attrs = %{end: ~N[2021-10-25 07:44:00], start: ~N[2021-10-25 07:44:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimeController.create_working_time(valid_attrs)
      assert working_time.end == ~N[2021-10-25 07:44:00]
      assert working_time.start == ~N[2021-10-25 07:44:00]
    end
    ...
    test "update_working_time/2 with valid data updates the working_time" do
      working_time = working_time_fixture()
      update_attrs = %{end: ~N[2021-10-26 07:44:00], start: ~N[2021-10-26 07:44:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimeController.update_working_time(working_time, update_attrs)
      assert working_time.end == ~N[2021-10-26 07:44:00]
      assert working_time.start == ~N[2021-10-26 07:44:00]
    end
